...
class AttentionSnapshot:
    ...
    confidence: float
    priority_bias: dict[str, float] = field(default_factory=lambda: {
        "performance": 1.0,
        "security": 1.0,
        "reflection": 1.0
    })
...
class AttentionEngine:
    ...
    def get_current_attention(self) -> AttentionSnapshot:
        ...
        elif active_window and any(x in active_window.lower() for x in ["vscode", "visual studio", "intellij", "pycharm", "sublime", "neovim"]):
        ...
        # Check for Research
        elif active_window and any(x in active_window.lower() for x in ["chrome", "brave", "firefox", "arxiv", "paper", "documentation"]):
        ...
        if state != self._last_state:
            ...
            log_event(logger, 20, "attention_state_changed", state=state.value, triggers=triggers)
            
        return snapshot

    def _build_snapshot(self, state: AttentionState, confidence: float, triggers: list[str], active_window: str | None) -> AttentionSnapshot:
        # Default modulation
    ...
    def _get_active_window_title(self) -> str | None:
        ...
        try:
            ...
            out_id = subprocess.check_output(["xprop", "-root", "_NET_ACTIVE_WINDOW"], stderr=subprocess.DEVNULL).decode()
            ...
            out_list = subprocess.check_output(["wmctrl", "-lp"], stderr=subprocess.DEVNULL).decode()
        ...
        try:
            with get_connection(self.db_path) as conn:
                conn.execute(
                    ...
                    "VALUES (?, ?, ?, ?, ?, ?)",
                    (
                        aid, s.state.value, s.confidence,
                        json.dumps(s.triggers), json.dumps({"active_window": s.active_window}),
    ...
    def get_history(self, limit: int = 50) -> list[dict]:
        with get_connection(self.db_path) as conn:
            rows = conn.execute(
                "SELECT * FROM cognitive_attention_history ORDER BY created_at DESC LIMIT ?", (limit,)
            ).fetchall()
