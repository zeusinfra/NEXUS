...
from zeus_core.v4.types import AutonomyMode, RiskLevel
...
_DESTRUCTIVE_PATTERNS: list[tuple[re.Pattern, RiskLevel, str]] = [
    (re.compile(r"\brm\b.*\b-rf\b.*\b/\b"), RiskLevel.CRITICAL, "remoção recursiva do root"),
    (re.compile(r"\bmkfs\b|\bdd\b\s+if="), RiskLevel.CRITICAL, "formatação/overwrite de disco"),
    (re.compile(r"\bshutdown\b|\breboot\b|\bpoweroff\b"), RiskLevel.HIGH, "interrupção do sistema"),
...
class SemanticGuardian:
    ...
    def assess_action(self, action: dict) -> RiskAssessment:
        ...
        if a_type == "cmd":
            ...
            if any(tok in cmd for tok in ["apt ", "dnf ", "pacman ", "pip install", "curl ", "wget "]):
                return RiskAssessment(RiskLevel.MEDIUM, "instalação/download pode alterar o sistema", "package_or_network")
        ...
        if a_type == "file":
            ...
            if op in {"delete", "rm"}:
                if path in {"/", "~", ""} or path.startswith("/etc") or path.startswith("/usr"):
                    return RiskAssessment(RiskLevel.CRITICAL, "deleção em path sensível", "file_delete")
                return RiskAssessment(RiskLevel.MEDIUM, "deleção de arquivo", "file_delete")
            if op in {"write", "move", "copy", "rename"} and (path.startswith("/etc") or path.startswith("/usr")):
                return RiskAssessment(RiskLevel.HIGH, "alteração em path sensível", "file_modify")
    ...
    def requires_confirmation(self, assessment: RiskAssessment) -> bool:
        ...
        if self.mode == AutonomyMode.DEV and assessment.level == RiskLevel.MEDIUM:
            return os.getenv("ZEUS_V4_CONFIRM_MEDIUM", "0").strip().lower() in {"1", "true", "yes", "on"}
        return False
