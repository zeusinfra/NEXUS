...
from zeus_core.events.event_bus import get_pending_events, mark_event_processed
from zeus_core.cognitive.classifier import decide_action
from zeus_core.integrations.notion import create_notion_page
from zeus_core.integrations.linear import create_linear_issue
from zeus_core.memory.sqlite_memory import link_external_resource
...
async def _process_event(event: dict):
    ...
    print(f"[SyncWorker] Processando evento ID {event_id}: {payload.get('title', 'Sem titulo')}")
    ...
    try:
        ...
        print(f"[SyncWorker] Decisão para '{payload.get('title')}': {action} ({decision.get('reason')})")
        ...
        if action in ['send_to_notion', 'both']:
            ...
            if 'id' in notion_resp:
                link_external_resource(obsidian_path=payload.get('path'), notion_id=notion_resp['id'])
        ...
        if action in ['create_linear_issue', 'both']:
            ...
            if 'id' in linear_resp:
                link_external_resource(obsidian_path=payload.get('path'), linear_id=linear_resp['id'])
