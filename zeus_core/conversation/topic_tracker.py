...
class TopicTracker:
    ...
    async def detect_topic_shift(self, user_message: str) -> str:
        ...
        if any(
            ph in msg_lower
            for ph in ["voltando", "como falamos antes", "sobre aquele assunto"]
        ):
            if len(self._topic_history) > 1:
                # Retorna ao tópico anterior (simplificado)
                self._current_topic_id = self._topic_history[-2]
                await event_bus.publish_async(
                    EventType.CONVERSATION_TOPIC_SHIFT,
                    {"new_topic": self._current_topic_id},
                )
                return self._current_topic_id
        ...
        if (
            "mudando de assunto" in msg_lower
            or "agora quero" in msg_lower
            or "outra coisa" in msg_lower
        ):
