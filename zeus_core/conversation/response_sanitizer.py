...
class ResponseSanitizer:
    ...
    async def sanitize(self, response: str) -> str:
        ...
        response = re.sub(r'--- OBJETIVO ATUAL ---.*?(?=---|$)', '', response, flags=re.DOTALL)
        response = re.sub(r'--- HISTÓRICO RECENTE.*?(?=---|$)', '', response, flags=re.DOTALL)
        response = re.sub(r'^(USER|ASSISTANT|SYSTEM):\s*', '', response, flags=re.IGNORECASE | re.MULTILINE)
        ...
        for p in paragraphs:
            ...
            if not p_clean:
                continue
            if p_clean in seen:
                await event_bus.publish_async(EventType.RESPONSE_DUPLICATED, {"content_preview": p_clean[:50]})
                continue
            seen.add(p_clean)
