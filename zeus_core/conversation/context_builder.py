...
from zeus_core.conversation.turn_store import Turn, turn_store
...
class ContextBuilder:
    ...
    async def build_context(self, session_id: str, current_message: str, system_prompt: str, 
                      active_goal: str = "", recent_errors: List[str] = None, 
                      topic_id: str = "general") -> List[Dict[str, str]]:
        ...
        if turns:
            hist_text = "\n".join([f"{t.role.upper()}: {t.content}" for t in turns[-5:]]) # Ultimos 5 turnos
            if len(hist_text) > hist_budget:
                hist_text = self._truncate(hist_text, hist_budget)
                await event_bus.publish_async(EventType.CONTEXT_TOO_LARGE, {"topic": topic_id})
