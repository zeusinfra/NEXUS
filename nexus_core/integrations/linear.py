...
def create_linear_issue(
    title: str, description: str, labels: list[str], priority: str, source_path: str
) -> dict:
    ...
    final_desc = (
        ...
        if validation.action == "sanitized"
    ...
    try:
        response = requests.post(
            GRAPHQL_URL, headers=_get_headers(), json=payload, timeout=10
        )
        ...
        if labels:
            print(
                f"[Linear] Lembrete: Labels {labels} não foram aplicadas nativamente (requer Label IDs)."
            )
...
def get_active_issues() -> list[dict]:
    ...
    try:
        response = requests.post(
            GRAPHQL_URL, headers=_get_headers(), json={"query": query}, timeout=10
        )
        response.raise_for_status()
        data = response.json()
        return (
