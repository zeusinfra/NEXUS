...
from nexus_core.conversation.turn_store import turn_store, Turn
from nexus_core.conversation.topic_tracker import topic_tracker
from nexus_core.conversation.context_builder import context_builder
from nexus_core.conversation.response_sanitizer import response_sanitizer
...
class ConversationManager:
    ...
    async def process_user_input(
        self, session_id: str, user_message: str, system_prompt: str
    ) -> List[Dict[str, str]]:
        ...
        user_turn = Turn.create(
            session_id=session_id, role="user", content=user_message, topic_id=topic_id
        )
