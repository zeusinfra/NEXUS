...
@pytest.mark.asyncio
async def test_service_name_accepts_common_systemd_units(
    root_daemon_instance, monkeypatch
):
    calls = []
...
@pytest.mark.asyncio
async def test_execute_safe_command_rejects_prefix_collision(
    root_daemon_instance, monkeypatch
):
    async def mock_to_thread(func, arg):
        pytest.fail("should not execute")
...
@pytest.mark.asyncio
async def test_execute_safe_command_allows_tokenized_read_only_command(
    root_daemon_instance, monkeypatch
):
    calls = []
