...
struct ZeusCoreService {
    system: Arc<<MutexMutex<<SystemSystem>>,
    mode: Arc<<MutexMutex<<StringString>>,
}
...
impl ZeusCoreService {
    ...
    fn is_command_safe(&self, command: &str) -> bool {
        let forbidden = ["rm -rf", "mkfs", "shutdown", "reboot", "sudo"];
...
impl ZeusCore for ZeusCoreService {
    async fn execute_action(
        &self,
        request: Request<<ActionActionRequest>,
    ) -> Result<<ResponseResponse<<ActionActionResponse>, Status> {
        ...
        Ok(Response::new(ActionResponse {
            success: output.status.map(|s| s.success()).unwrap_or(false),
    ...
    async fn simulate_action(
        &self,
        request: Request<<SimulationSimulationRequest>,
    ) -> Result<<ResponseResponse<<SimulationSimulationResponse>, Status> {
        let req = request.into_inner();
        ...
        Ok(Response::new(SimulationResponse {
            ...
            error: if success { "".into() } else { "Potential risk detected".into() },
        }))
    }

    async fn stream_telemetry(
        &self,
        _request: Request<<TelemetryTelemetryRequest>,
    ) -> Result<<ResponseResponse<<toktokio_stream::wrappers::ReceiverStream<<ResultResult<<TelemetryTelemetryUpdate, Status>>>, Status> {
            ...
            loop {
                {
                    ...
                    let update = TelemetryUpdate {
                        ...
                        disk_usage: 0.0, // Implementation for disk
                        processes: vec![], // Mapping processes here
                    };
    ...
    async fn set_mode(
        &self,
        request: Request<<ModeModeRequest>,
    ) -> Result<<ResponseResponse<<ModeModeResponse>, Status> {
        let req = request.into_inner();
...
#[tokio::main]
async fn main() -> Result<(), Box<<dyndyn std::error::Error>> {
