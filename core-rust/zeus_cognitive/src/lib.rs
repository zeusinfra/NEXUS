...
impl CognitiveEngineRust {
    fn score_goal(&self, goal: &Goal, context: &Value) -> f32 {
        ...
        let attention_state = context.get("attention").and_then(|a| a.get("state")).and_then(|v| v.as_str()).unwrap_or("idle");
...
impl CognitiveEngineRust {
    ...
    pub fn orchestrate(&self, goals_json: String, context_json: String, max_active: usize) -> PyResult<String> {
        let goals: Vec<Goal> = serde_json::from_str(&goals_json)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(format!("Invalid Goals JSON: {}", e)))?;
        ...
        if let Some(sys) = sys {
            let cpu = sys.get("cpu_percent").and_then(|v| v.as_f64()).unwrap_or(0.0);
            let ram = sys.get("ram_percent").and_then(|v| v.as_f64()).unwrap_or(0.0);
        ...
        // 2. Score goals
        let mut scored_goals: Vec<(f32, Goal)> = goals.into_iter().map(|g| {
        ...
        // 4. Split
        let selected: Vec<Goal> = scored_goals.iter().take(capacity).map(|x| x.1.clone()).collect();
        let deferred: Vec<Goal> = scored_goals.iter().skip(capacity).map(|x| x.1.clone()).collect();
