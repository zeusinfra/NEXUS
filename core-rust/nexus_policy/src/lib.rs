...
#[pymodule]
fn nexus_policy(m: &Bound<'_, PyModule>) -> PyResult<()> {
...
mod tests {
    ...
    fn with_env<F: FnOnce()>(autonomy: &str, allowlist: &str, f: F) {
        ...
        std::env::set_var("NEXUS_AUTONOMY_LEVEL", autonomy);
        std::env::set_var("NEXUS_CMD_ALLOWLIST", allowlist);
        f();
        std::env::remove_var("NEXUS_AUTONOMY_LEVEL");
        std::env::remove_var("NEXUS_CMD_ALLOWLIST");
    }
