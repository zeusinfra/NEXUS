...
class ResourceControl:
    ...
    def calculate_adaptive_delay(self, current_delay: float) -> float:
        ...
        if self.is_critical(snapshot):
            ...
            self.logger.warning(
                f"CRITICAL PRESSURE: Throttling loop. CPU: {cpu}%, RAM: {ram}%"
            )
    ...
    def is_critical(self, snapshot: Dict[str, float] = None) -> bool:
        ...
        if snapshot is None:
            snapshot = self.get_system_snapshot()
        return (
            snapshot["cpu"] > self.CPU_CRITICAL or snapshot["ram"] > self.RAM_CRITICAL
        )
    ...
    async def monitor_and_report(self):
        ...
        while True:
            ...
            if (
                snapshot["cpu"] > self.CPU_CRITICAL
                or snapshot["ram"] > self.RAM_CRITICAL
            ):
                self.logger.error(
                    f"Resource Exhaustion Warning: CPU {snapshot['cpu']}% RAM {snapshot['ram']}%"
                )
